<template>
	<el-dialog @open="init" :show-close='false' :close-on-click-modal='false' :visible.sync="video_preview_show" top="10%"
	 class="video-preview" width="50%">
		...
		<video-player v-if="showVideo" style="width:96%" class="video-player vjs-custom-skin" ref="videoPlayer" :playsinline="true"
	...
	export default {
		...
		methods: {
			close_preview: function() {
				this.$emit("close_preview", "video_preview_show");
				this.showVideo = false;
			},
			init() {
				this.showVideo = true;
				this.playerOptions.sources[0].src = this.media_preview_path;
