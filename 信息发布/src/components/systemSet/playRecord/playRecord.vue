<template>
  <div class="playRecord br bx">
    <div class="item">
	    	<el-input
			    :placeholder='$t("message.systemSet.taskName")'
			    prefix-icon="el-icon-search"
			    v-model="taskName"
			    @keyup.enter.native="keyEnter">
			  </el-input>
				<label>&emsp;{{$t("message.systemSet.publishDate")}}</label>
         ...
         <el-button class="mybtn-primary" type="primary" id="search-log" @click="handleDownload">{{$t("message.systemSet.exportBtn")}}</el-button> -->       
         <el-button class="mybtn-primary" type="primary" id="search-log" v-on:click="serachRecord">{{$t("message.systemSet.searchBtn")}}</el-button>
   	 </div>
   	 <!--列表-->
    <el-table  :data="records" class="my-table" @sort-change="sortChange"  @selection-change="selsChange" highlight-current-row v-loading="listLoading" height="77%" style="width: 100%; ">
      ...
      <el-table-column prop="scheduleName"    :label='$t("message.systemSet.taskName")' show-overflow-tooltip  style="width: 10%;" ></el-table-column>
      <el-table-column prop="layout"    :label='$t("message.systemSet.layout")' show-overflow-tooltip style="width: 5%;" ></el-table-column>      
      <el-table-column prop="userName"    :label='$t("message.systemSet.Publisher")' show-overflow-tooltip  style="width: 10%;" >
      	<template slot-scope="scope">
      		<div class='cell' v-if='scope.row.userName'>{{ scope.row.userName}}</div>
					<div v-else class='cell el-tooltip'>{{ scope.row.userName}}</div>      		
      	</template>
      </el-table-column>
   
      <el-table-column prop="createTime" :label='$t("message.systemSet.publishTime")' show-overflow-tooltip style="width: 20%;">
      	  <template slot-scope="scope">
	      	  <div class='cell' v-if='scope.row.createTime'>{{ scope.row.createTime}}</div>
						<div v-else class='cell el-tooltip'>{{ scope.row.createTime}}</div> 
          </template>
      </el-table-column>
      <el-table-column prop="display" :label='$t("message.systemSet.releaseTerminal")' show-overflow-tooltip  style="width: 10%;" ></el-table-column>
      <el-table-column prop="scheduleStart" :label='$t("message.systemSet.startTime")' show-overflow-tooltip  style="width: 20%;" ></el-table-column>
      <el-table-column prop="recurrenceRange" :label='$t("message.systemSet.endTime")' show-overflow-tooltip style="width: 20%;" ></el-table-column>
      <!--<el-table-column  :label='$t("message.systemSet.action")'  style="width: 10%;">
      	<template slot-scope="scope">					
					<el-button class="single-del" type="danger" size="small" @click="handleDel(scope.$index, scope.row)">{{$t("message.systemSet.singleDel")}}</el-button>
				</template>
      </el-table-column>-->
      <div slot="empty">
...
//	    },
         ...
         _this.searchTaskName = this.taskName;
				_this.getRecords();
			},
      getRecords() { //获取数据
        let _this = this;
        ...
        _this.taskName = this.searchTaskName;
        if(start_time) {					  
		       ...
		       this.refleshLoading = false;
				...
				console.log(_this.timestampToTime(start_time));//格式化日期
      	 _this.LonbonAjax({    	 
              type: 'GET',
              url: '/api/v1/scheduleEvent',
              data:{
              	limit: this.limit,
              	page: this.page,
              	searchStart: start_time,
              	searchEnd: end_time,
              	searchName:this.taskName,
              	//timeOrder: this.sort,
              }
            }).then((res) => { //成功
              ...
              this.total = res.data.total;
							...
							this.records.forEach(function(item, index) {
								let display=''; //发布终端
								_this.records[index]['createTime'] = _this.timestampToTime(_this.records[index]['createTime']);
								_this.records[index]['scheduleStart'] = _this.timestampToTime(_this.records[index]['scheduleStart']);
								_this.records[index]['recurrenceRange'] = _this.timestampToTime(_this.records[index]['recurrenceRange']);
								if(_this.records[index]['display'].length != 1){ 
									_this.records[index]['display'].forEach(function(item, n){
										display =display+ _this.records[index]['display'][n]+'、';
									});
								}else{// 如果只有一台设备
									display =_this.records[index]['display'][0];
								}
																
								_this.records[index]['display']=display;
							})							
							this.listLoading = false;
							this.searchLoading = false;
							this.refleshLoading = false;
