...
var VERSION = "1.7.0";
var base = window.localStorage["rootUrl"] ;
var baseMedia = window.localStorage["basemedia:port"];
...
function layout(id) {
	...
	self.parseXlf = function(data) {
		...
		console.log(self.regionObjects)
		playLog(4, "debug", "Layout " + self.id + " has " + self.regionObjects.length + " regions");
		self.ready = true;
		PRELOAD.addFiles('loader.gif');
		PRELOAD.on('finish', self.run);
		//      console.log(PRELOAD.on('finish', self.run)._listeners.finish[1]())
		//      PRELOAD.on('finish', self.run);
	...
	console.log(base + '/api/v1/getXlf')
	$.get(base + '/api/v1/getXlf', {
		layoutid: self.id
	}, self.parseXlf);
}
...
function region(parent, id, xml) {
	...
	self.transitionNodes = function(oldMedia, newMedia) {
		...
		if(newMedia.mediaType == 'text') {
			var src = $("#" + newMedia.containerName).children('iframe').attr('src');
			$("#" + newMedia.containerName).children('iframe').attr('src', src);
		}
	}
...
// 	if($(self.xml).find('text').length > 0) {
// 		self.text = $(self.xml).find('text').html().replace(']]&gt;', '').replace('[Clock|DD/MM/YYYY]', getNowTime());
// 	}
	...
	if(self.options.backgroundcolor){
        $("#" + self.containerName).css("background-color", self.options.backgroundcolor);
	}
	...
	if(self.render == "html") {
		...
		var tmpUrl = baseMedia + self.options['uri'];
		//      +"?id="+self.containerName;;
		console.log(tmpUrl)
		PRELOAD.addFiles(tmpUrl);
		// PRELOAD.addFiles( 'http://im.cc:84/media/5abf4ce5175c0.jpg');
		// http://im.cc:84/api/v1/getXlf
	...
	} else if(self.mediaType == "text" || self.mediaType == "webpage" || self.mediaType == "datasetview"  || self.mediaType == "embedded") {
		// $("#" + self.containerName).append('<iframe scrolling="no" id="' + self.iframeName + '" src="index.php?p=module&mod=' + self.mediaType + '&q=Exec&method=GetResource&raw=true&preview=true&layoutid=' + self.region.layout.id + '&regionid=' + self.region.id + '&mediaid=' + self.id + '&lkid=&width=' + self.divWidth + '&height=' + self.divHeight + '" width="' + self.divWidth + 'px" height="' + self.divHeight + 'px" style="border:0;"></iframe>');
    $("#" + self.containerName).append('<iframe scrolling="no" id="' + self.iframeName + '" src="' + base + '/api/v1/preview?type=' + self.mediaType + '&q=Exec&method=GetResource&raw=true&preview=true&layoutid=' + self.region.layout.id + '&regionid=' + self.region.id + '&mediaid=' + self.id + '&lkid=&width=' + self.divWidth + '&height=' + self.divHeight + '" width="' + self.divWidth + 'px" height="' + self.divHeight + 'px" style="border:0;"></iframe>');
	}
	...
	} else if(self.mediaType == 'live') {
    var tmpUrl = self.options['channelurl'];
    var width = $('#'+self.containerName).width();
    var height = $('#'+self.containerName).height();
    var videoId = self.containerName+'video';
    // rtmp://192.168.3.230:1935/hls/demo
    console.log(tmpUrl)
//     $('#'+self.containerName).append('<video id='+videoId+' class="video-js" controls autoplay="autoplay" preload="auto" width='+width+' height='+height+''+
//     'poster="http://ppt.downhot.com/d/file/p/2014/08/12/9d92575b4962a981bd9af247ef142449.jpg" >  '+
//      '<source src='+tmpUrl+' type="rtmp/flv">  '+
...
//    })
	  ...
	  // ckplayer
		var videoObject = {
			container: '#'+self.containerName, //容器的ID或className
